...
# --- 1) point docker data-root at the data volume (reuse image, no pull) -----
DJ=/etc/docker/daemon.json
if ! grep -q "$DATA/var/lib/docker" "$DJ" 2>/dev/null; then
  log "setting docker data-root -> $DATA/var/lib/docker"
...
docker image inspect "$IMG" >/dev/null 2>&1 || { log "image missing -> docker pull $IMG"; docker pull "$IMG"; }
...
for port in 7000 7001 7002 7003 7006 7007; do
  ...
  docker exec "$CON" bash -lc "curl -sf -m4 http://127.0.0.1:$port/health >/dev/null 2>&1" && log ":$port ready"
done
