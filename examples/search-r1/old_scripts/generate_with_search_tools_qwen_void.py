"""Search-R1 generate function with void masking.

Based on generate_with_search_tools_qwen.py, but with a key difference:
when the model outputs neither a valid tool call nor an <answer> block,
the void turn is added to context normally, but the entire sequence's
loss_mask is zeroed out so it does not contribute to training.

Void masking metrics are tracked in sample.metadata:
- void_masked: 1 if the sequence was masked due to void output, 0 otherwise
"""
...
SEARCH_R1_CONFIGS = {
    ...
    "mask_no_tool_call_warmup": 12,  # start masking after this many rollouts
}
...
class MessageContextWindowManager:
    ...
    def build_training_data(
        ...
        for i, turn in enumerate(self.turns):
            ...
            # sglang may output <|endoftext|> as EOS; replace with <|im_end|>
            # to match Qwen chat format (same logic as build_context).
            # On the last turn, replace with <|im_end|><|endoftext|>.
            cur_token_ids = list(turn["model_token_ids"])
            cur_log_probs = list(turn["model_log_probs"]) if return_logprob else None
            for j in range(len(cur_token_ids)):
                if cur_token_ids[j] == endoftext_id:
                    if is_last:
                        cur_token_ids[j] = im_end_id
                        cur_token_ids.insert(j + 1, endoftext_id)
                        if cur_log_probs is not None:
                            cur_log_probs.insert(j + 1, 0.0)
                    else:
                        cur_token_ids[j] = im_end_id
                    break  # only replace the first (EOS) occurrence
...
async def generate(args, sample: Sample, sampling_params) -> Sample:
    ...
    for _turn_idx in range(config["max_turns"]):
        ...
        if action == "answer":
            ...
            # Void output: add the turn but mark sequence for masking
            has_void = True
            logger.info(
                f"Void output at turn {_turn_idx}, sequence will be masked. "
                f"Response preview: {cur_response[:200]!r}"
            )
            manager.add_turn(
                cur_response, cur_token_ids, cur_log_probs, search_result=None
            )
    ...
    sample.metadata["void_masked"] = int(has_void)

    # Zero out loss_mask if any void output was detected
    if has_void:
        loss_mask = [0] * len(loss_mask)
        logger.info("Void output detected, masking entire sequence.")

    logger.debug(
        f"MessageContextWindow stats: turns={stats['num_turns']}, "
        f"obs_full={stats['obs_full']}, obs_compressed={stats['obs_compressed']}, "
        f"void_masked={int(has_void)}"
    )
